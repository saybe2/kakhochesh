...
class MapApp(QMainWindow):
    ...
    MIN_LON = -180.0
    MAX_LON = 180.0
    SEARCH_RADIUS_METERS = 50  # Радиус поиска организаций в метрах
    ...
    def init_ui(self):
        ...
        # Добавляем поле для отображения информации об организации
        org_layout = QHBoxLayout()
        org_label = QLabel("Организация:")
        self.org_display = QLineEdit()
        self.org_display.setReadOnly(True)
        self.org_display.setPlaceholderText("Нажмите правой кнопкой мыши на карте для поиска организации")
        org_layout.addWidget(org_label)
        org_layout.addWidget(self.org_display)
    ...
    def mousePressEvent(self, event: QMouseEvent):
        """Обработка кликов мыши по карте"""
        ...
        if (0 <= map_label_pos.x() <= self.map_label.width() and 
            ...
            # Получаем географические координаты клика
            lon, lat = self.get_geo_coordinates_from_click(map_label_pos)
            
            if lon is not None and lat is not None:
                if event.button() == Qt.MouseButton.LeftButton:
                    # Левый клик - поиск объекта по координатам
                    self.search_by_coordinates(lat, lon)
                elif event.button() == Qt.MouseButton.RightButton:
                    # Правый клик - поиск организации
                    self.search_organization_by_coordinates(lat, lon)

    def get_geo_coordinates_from_click(self, click_pos):
        """Получает географические координаты из позиции клика"""
        ...
        click_x = click_pos.x() - x_offset
        click_y = click_pos.y() - y_offset
        ...
        if 0 <= click_x <= pixmap.width() and 0 <= click_y <= pixmap.height():
            ...
            return self.pixel_to_geo(click_x, click_y, pixmap.width(), pixmap.height())
        return None, None
    ...
    def haversine_distance(self, lat1, lon1, lat2, lon2):
        """Вычисляет расстояние между двумя точками на сфере в метрах (формула гаверсинуса)"""
    ...
    def search_organization_by_coordinates(self, lat, lon):
        """Поиск организации по координатам в радиусе 50 метров"""
        try:
            # Сбрасываем предыдущие результаты поиска
            self.reset_search(keep_map_center=True)
            
            # URL для поиска организаций через Yandex Search API
            ...
            for feature in features:
                ...
                if len(coordinates) == 2:
                    org_lon, org_lat = coordinates
                    # Вычисляем расстояние от точки клика до организации
                    distance = self.haversine_distance(lat, lon, org_lat, org_lon)
                    
                    if distance <= self.SEARCH_RADIUS_METERS and distance < min_distance:
                        properties = feature.get("properties", {})
                        company_meta = properties.get("CompanyMetaData", {})
                        
                        org_name = company_meta.get("name", "Неизвестная организация")
                        org_address = company_meta.get("address", "")
                        org_hours = company_meta.get("Hours", {})
                        org_phones = company_meta.get("Phones", [])
            ...
            if found_org and min_distance <= self.SEARCH_RADIUS_METERS:
                # Формируем информацию об организации
                org_info = f"{found_org['name']} ({min_distance:.0f} м)"
                if found_org['address']:
                    org_info += f" - {found_org['address']}"
                
                # Сохраняем информацию об организации
                self.current_organization = found_org
                self.org_display.setText(org_info)
                
                # Устанавливаем метку на карте в точку найденной организации
                self.search_point = found_org['coordinates']
                self.found_address = found_org['address'] if found_org['address'] else found_org['name']
                
                # Обновляем адрес (если найден)
                if self.found_address:
                    self.address_display.setText(self.found_address)
                
                # Обновляем координаты в полях ввода
                org_lon, org_lat = found_org['coordinates']
                self.lat_input.setText(f"{org_lat:.6f}")
                self.lon_input.setText(f"{org_lon:.6f}")
                
                # Перерисовываем карту с меткой организации
                self.show_map()
            ...
            self.org_display.setText("Ошибка поиска")
        except Exception as exc:
            QMessageBox.critical(self, "Ошибка", f"Не удалось найти организацию:\n{exc}")
            self.org_display.setText("Ошибка")

    def search_by_coordinates(self, lat, lon):
        """Поиск объекта по координатам (обратное геокодирование)"""
        try:
            # Сбрасываем информацию об организации при обычном поиске
    ...
    def get_map_image(self, lat, lon, zoom):
        try:
            ...
            if self.search_point is not None:
                ...
                # Разные цвета меток для обычного поиска и организаций
                if self.current_organization:
                    request_url += f"&pt={point_lon},{point_lat},pm2gnm"  # Зеленая метка для организаций
                else:
                    request_url += f"&pt={point_lon},{point_lat},pm2rdm"  # Красная метка для обычного поиска
    ...
    def search_object(self):
        ...
        try:
            # Сбрасываем информацию об организации
            self.current_organization = None
            self.org_display.clear()
    ...
    def reset_search(self, keep_map_center=False):
        ...
        if not keep_map_center:
            self.show_map()
        else:
            # Только перерисовываем карту без изменения центра
            self.show_map()

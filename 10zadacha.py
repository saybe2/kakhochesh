...
class MapApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.last_search_query = None  # Сохраняем последний поисковый запрос
        self.last_search_coords = None  # Сохраняем координаты последнего найденного объекта
    ...
    def on_postal_checkbox_changed(self, state):
        ...
        if self.search_point is not None and self.last_search_coords is not None:
            # Обновляем адрес с учетом нового состояния чекбокса
            ...
            # Если есть адрес, но нет сохраненных координат, используем текущие координаты
    ...
    def update_address_for_coords(self, lon, lat):
        ...
        try:
            # Получаем базовый адрес через геокодер
    ...
    def find_object_center_and_address(self, query):
        ...
        # Сохраняем координаты для возможного обновления адреса
